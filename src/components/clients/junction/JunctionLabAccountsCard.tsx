...
export function JunctionLabAccountsCard({ detail, env, provisioned, onManage }: Props) {
  const mode = detail.lab_accounts.mode ?? "platform"
  ...
  return (
    <div className="bg-card border rounded-2xl p-5 sm:p-6 shadow-sm flex flex-col justify-between gap-4 hover:shadow-md transition-shadow">
      <div className="flex items-start gap-3.5">
        ...
        <div>
          <h3 className="text-sm font-bold">Lab Account Mode</h3>
          <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
            {isPlatformMode
              ? "Orders route through Junction platform lab accounts based on the selected lab test and provider. Tenant lab-account linking is not required."
      ...
      <div className="bg-muted/30 rounded-xl p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border">
        <span className="text-xs font-medium text-muted-foreground">
          {isPlatformMode
            ? "Junction platform accounts enabled"
        ...
        >
          {isPlatformMode ? "View org lab accounts" : "Manage lab accounts"} <ChevronRight className="w-3.5 h-3.5" />
        </button>
      </div>
    </div>
  )
