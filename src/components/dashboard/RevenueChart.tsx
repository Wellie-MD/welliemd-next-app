...
function RevenueChart({ data, subtitle = "Rolling 12 months" }: RevenueChartProps) {
  ...
  return (
      <Card className="rounded-2xl border-border/70 bg-gradient-to-br from-primary/10 via-background to-cyan-50/40 shadow-sm w-full">
      <CardHeader className="flex flex-row items-start justify-between">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className="h-2 w-2 rounded-full bg-[#3CC3DF]" />
            <span>Revenue</span>
              ...
              <ResponsiveContainer width="100%" height="100%">
                {chartMode === "area" ? (
                  <AreaChart data={data} margin={{ top: 12, right: 22, left: 8, bottom: 4 }}>
                    <defs>
                      <linearGradient id="revenueAreaGradientAdmin" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#3CC3DF" stopOpacity={0.35} />
                        <stop offset="95%" stopColor="#3CC3DF" stopOpacity={0.05} />
                      </linearGradient>
                    </defs>
                      ...
                      content={({ active, payload, label }) => {
                        ...
                        return (
                          <div className="rounded-lg border bg-white/95 px-3 py-2 text-xs shadow-lg">
                            ...
                            <div className="mt-1 text-gray-700">Revenue: {formatCurrencyTick(value)}</div>
                    ...
                    <Area
                      ...
                      stroke="#3CC3DF"
                      strokeWidth={3}
                      fill="url(#revenueAreaGradientAdmin)"
                      fillOpacity={1}
