...
export function NewPatientChart({ data, subtitle = "Rolling 12 months" }: NewPatientChartProps) {
  ...
  const values = data.map((point) => Number(point.new_patients) || 0)
  ...
  return (
    <Card className="rounded-2xl border-border/70 bg-gradient-to-br from-primary/10 via-background to-rose-50/35 dark:to-slate-900/40 shadow-sm w-full h-full">
      <CardHeader className="flex flex-row items-start justify-between">
        <div>
          <CardTitle className="text-gray-800 dark:text-slate-100">New Patients</CardTitle>
              ...
              <ResponsiveContainer width="100%" height="100%">
                {chartMode === "area" ? (
                  <AreaChart data={data} margin={{ top: 12, right: 22, left: 8, bottom: 4 }}>
                    <defs>
                      <linearGradient id="patientsAreaGradientClient" x1="0" y1="0" x2="0" y2="1">
                      ...
                      content={({ active, payload, label }) => {
                        ...
                        return (
                          <div className="rounded-lg border bg-white/95 dark:bg-slate-900 px-3 py-2 text-xs shadow-lg">
                            ...
                            <div className="mt-1 text-gray-700 dark:text-slate-300">New Patients: {Number(payload[0].value || 0).toLocaleString()}</div>
                    ...
                    <Area
                      ...
                      stroke="#FF928A"
                      strokeWidth={3}
                      fill="url(#patientsAreaGradientClient)"
                      fillOpacity={1}
