...
export function showFloatingToast({ title, description }: FloatingToastOptions) {
  return toast({
    title: <span className="block w-full text-center text-sm font-semibold">{title}</span>,
    description: description ? (
      <span className="block w-full text-center text-xs font-medium opacity-90">{description}</span>
    ) : undefined,
