...
export const fetchOrders = async (
  ...
  try {
    const { data } = await api.get<PaginatedOrdersResponse>(ENDPOINT, {
      params,
    });
    console.log({ data });
...
export const fetchOrdersByPatient = async (
  patientId: string,
  params?: Record<string, unknown>,
): Promise<PaginatedOrdersResponse> => {
  try {
    const { data } = await api.get<PaginatedOrdersResponse>(ENDPOINT, {
      params: { ...params, patient_id: patientId },
    });
    return data;
  } catch (error) {
...
export const fetchOrderByOrderId = async (orderId: string): Promise<Order> => {
  try {
    const { data } = await api.get<Order>(
      `${ENDPOINT}by_order_id/${encodeURIComponent(orderId)}/`,
    );
    return data;
  } catch (error) {
...
export const refundOrder = async (
  ...
  try {
    const { data } = await api.post<OrderRefundResponse>(
      `${ENDPOINT}${id}/refund/`,
      payload,
    );
    return data;
