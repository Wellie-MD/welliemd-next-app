...
/** Serialize refreshes in this tab; the API's short replay receipt covers other tabs. */
export const withRefreshLock = async <T>(portal: string, work: RefreshWork<T>): Promise<T> => {
  ...
  const run = async (): Promise<T> => {
    ...
    if (lockManager) {
      return lockManager.request(`welliemd-auth-refresh:${portal}`, { mode: 'exclusive' }, work);
    }
    return work();
  };
