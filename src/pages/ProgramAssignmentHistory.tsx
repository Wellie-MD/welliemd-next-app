...
import { programAssignmentHistoryApi, ProgramAssignmentLog, AssignmentHistoryParams } from "@/api/treatments";
...
const getColumns = () => [
  ...
  },
  {
    key: "error_message",
    label: "Error",
    render: (value: string | undefined, row: ProgramAssignmentLog) => {
...
export default function ProgramAssignmentHistory() {
  const navigate = useNavigate();
  const [logs, setLogs] = useState<ProgramAssignmentLog[]>([]);
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      const response = await programAssignmentHistoryApi.getHistory(params);
      setLogs(response.results);
      setPagination((prev) => ({ ...prev, total: response.count }));
      // Compute status breakdown from paginated results (note: only shows current page distribution)
      const counts = {
  ...
  const handleExport = useCallback(() => {
    ...
    exportToCSV(logs, exportColumns, "program_assignment_history");
    toast({
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Program Assignment History</h1>
            <p className="text-sm text-muted-foreground mt-1">
              View all program assignment records and their status
            </p>
          </div>
        </div>
      ...
      <DataTable
        data={logs}
        columns={columns}
        searchPlaceholder="Search by program, client, or assigned by"
        showDatePicker={true}
