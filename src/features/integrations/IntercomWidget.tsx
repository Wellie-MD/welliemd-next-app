...
// Conversations REST API as the signed-in user. Agent replies arrive in real
// time over the existing Channels websocket (an Intercom webhook pushes a
// `support_message` event), with a poll backstop for when the socket is down.
//
// Supports starting a new chat and going back to previous conversations. Copy
// is adapted for the patient audience.
//
// HIPAA: the footer reminds patients not to share sensitive medical details.
// ─────────────────────────────────────────────────────────────────────────────
...
function buildSupportWsUrl(): string | null {
  ...
  if (env.VITE_WS_BASE_URL) {
    origin = env.VITE_WS_BASE_URL.replace(/\/+$/, '');
  } else {
    try {
  ...
  return `${origin}/ws/patient/notifications/?token=${encodeURIComponent(token)}`;
}
...
/** Best-effort teardown of legacy Intercom Messenger DOM (parity with client). */
...
// ── Reference UI content (patient audience) ────────────────────────
...
const QUICK_REPLIES: { key: string; label: string; prefill: string }[] = [
  ...
  { key: 'medication', label: 'My medication', prefill: 'I have a question about my medication.' },
  { key: 'orders', label: 'My order', prefill: 'I have a question about my order.' },
...
export const IntercomWidget = () => {
  ...
  const onLauncherPointerDown = (e: React.PointerEvent<HTMLButtonElement>) => {
    const current = widgetPos || { right: 24, bottom: 24 };
  ...
  const getConversationData = async (id: string | null): Promise<ConversationResponse | null> => {
    ...
    try {
      const res = await apiClient.get<ConversationResponse>(url);
      return res.data;
    } catch {
      return null;
    }
  ...
  const fetchList = async () => {
    try {
      const res = await apiClient.get<{ conversations: ConversationSummary[] }>(LIST_URL);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      let changed = false;
      try {
        const res = await apiClient.get<{ updated_at: number }>(UPDATES_URL);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      const res = await apiClient.post<ConversationResponse>(CONVERSATION_URL, {
        message: body,
    ...
    >
      ...
      <div className="ic-panel" role="dialog" aria-label={`${displayName} Support`}>
        <div className="ic-head">
          ...
          <div className="ic-title">{view === 'list' ? 'Your conversations' : `${displayName} Support`}</div>
          {view === 'thread' && (
        ...
        <div className="ic-foot">
          ...
          Secured · please don’t share sensitive medical details here
        </div>
      </div>
