  ...
  return (
    <div className="min-h-full bg-[#f5f7fb] p-6 text-slate-950 dark:bg-[#0f1117] dark:text-slate-50 lg:p-8">
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6">
        ...
        <section>
          <div className="flex w-full max-w-full flex-wrap gap-1 self-start rounded-xl border border-slate-100 bg-slate-100/70 p-1 dark:border-slate-700 dark:bg-slate-800/50">
            {(["all","intake","follow_up"] as const).map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`flex-1 rounded-lg px-4 py-2 text-xs font-semibold transition-all cursor-pointer whitespace-nowrap md:flex-none ${
                  activeTab === tab
