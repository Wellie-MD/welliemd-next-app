...
export function QuestionnairePreviewDialog({
  ...
  const previewUrl = useMemo(
    () => withShellContext(buildQuestionnairePreviewUrl(previewContext), identity),
    [identity, previewContext],
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[min(600px,calc(100dvh-32px))] w-[min(512px,calc(100vw-32px))] max-w-none flex-col gap-0 overflow-hidden rounded-xl border border-border bg-card p-0 shadow-2xl [&>button]:hidden">
        <DialogHeader className="shrink-0 border-b border-border-light px-5 py-4 text-left">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <DialogTitle className="text-base font-semibold text-foreground">
                Patient Preview
              </DialogTitle>
              <DialogDescription className="mt-1 truncate text-xs text-text-tertiary">
                {subtitle}
              </DialogDescription>
            </div>
            <button
              type="button"
              className="rounded-md p-1 text-text-tertiary outline-none transition hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
        ...
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border-light px-5 py-2.5">
        ...
        <div className="min-h-0 flex-1 bg-background p-3">
          <div className="relative h-full overflow-hidden rounded-lg border border-border-light bg-white">
            {status === "loading" ? (
