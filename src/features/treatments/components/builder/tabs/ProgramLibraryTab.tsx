import { Button } from "@/components/ui/button";
import { Plus } from "lucide-react";
import type { Program } from "../../../types";

interface ProgramLibraryTabProps {
  programs: Program[];
  onAddItem: (item: {
    kind: "program";
    title: string;
    subtitle: string;
    treatmentTypeKey?: string;
  }) => void;
}

export function ProgramLibraryTab({ programs, onAddItem }: ProgramLibraryTabProps) {
  return (
    <div className="space-y-3 mt-4">
      {programs.map((program) => (
        <div
          key={program.id}
          onClick={() =>
            onAddItem({
              kind: "program",
              title: program.name,
              subtitle: "Treatment program questions.",
              treatmentTypeKey: program.treatmentTypeKey,
            })
          }
          className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:border-[#12517A] cursor-pointer flex justify-between items-center transition-colors group"
        >
          <div>
            <div className="font-semibold text-slate-900 group-hover:text-[#12517A]">{program.name}</div>
            <div className="text-xs text-slate-500 mt-1">{program.questionCount} clinical questions</div>
          </div>
          <Button variant="ghost" size="icon" className="shrink-0 text-slate-400 group-hover:text-[#12517A]">
            <Plus className="h-5 w-5" />
          </Button>
        </div>
      ))}
    </div>
  );
}
