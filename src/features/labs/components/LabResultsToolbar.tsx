...
export default function LabResultsToolbar({ count, filter, sort, onFilter, onSort }: Props) {
  return <div className="km-lab-results-toolbar">
    <div><span className="km-lab-section-label">Recent results</span><div className="km-lab-filter-pills">{(['all', 'normal', 'flagged'] as ResultFilter[]).map((value) => <button type="button" key={value} className={filter === value ? 'is-active' : ''} onClick={() => onFilter(value)}>{value}</button>)}</div></div>
