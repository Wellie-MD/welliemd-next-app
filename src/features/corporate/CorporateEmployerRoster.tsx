...
export default function CorporateEmployerRoster() {
  ...
  return <div className="space-y-6 p-4 sm:p-6 lg:p-8">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div className="mb-2 flex gap-2"><Badge>Day 2 live</Badge><Badge variant="outline">Employer Admin</Badge></div><h1 className="text-2xl font-bold">Employee roster</h1><p className="mt-1 text-sm text-muted-foreground">Aggregate participation visibility for {context.name}.</p></div><Badge variant="secondary"><ShieldCheck className="mr-1 h-3.5 w-3.5" />No PHI or individual clinical status</Badge></div>
