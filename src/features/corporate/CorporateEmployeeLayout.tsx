...
function CorporateNav({ close }: { close?: () => void }) {
  return <nav className="space-y-1 p-3">{items.map(({ label, path, icon: Icon }) => <NavLink key={path} to={path} onClick={close} className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition ${isActive ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}><Icon className="h-4 w-4" />{label}</NavLink>)}<div className="mt-4 space-y-2 border-t pt-4"><button disabled aria-disabled="true" className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-slate-400"><BookOpen className="h-4 w-4" />Education <span className="ml-auto text-[10px]">Day 2</span></button></div></nav>;
}
...
export default function CorporateEmployeeLayout() {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-950">
    <header className="fixed inset-x-0 top-0 z-40 flex h-16 items-center justify-between border-b bg-white/95 px-4 backdrop-blur lg:px-6"><div className="flex items-center gap-3"><button className="rounded-lg border p-2 lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open navigation"><Menu className="h-4 w-4" /></button>{logos?.square ? <img src={logos.square} alt="Employer program" className="h-8 w-auto" /> : <span className="font-semibold">WellieMD Corporate</span>}</div><div className="text-right"><p className="text-[10px] font-semibold uppercase tracking-wider text-blue-700">Pilot preview</p><p className="text-xs text-slate-500">Employee program portal</p></div></header>
