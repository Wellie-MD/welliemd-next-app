...
export default function CorporateWorkspace() {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><BookOpen className="h-4 w-4" />Assign a program to an employer</CardTitle></CardHeader><CardContent><div className="grid gap-3 lg:grid-cols-[1fr_1fr_auto] lg:items-end"><div><p className="mb-2 text-xs font-medium text-muted-foreground">Employer</p><Select value={employerId} onValueChange={setEmployerId}><SelectTrigger><SelectValue placeholder="Choose an employer" /></SelectTrigger><SelectContent>{context.available_employers.map((employer) => <SelectItem key={employer.id} value={employer.id}>{employer.name}</SelectItem>)}</SelectContent></Select></div><div><Select value={programId} onValueChange={setProgramId}><SelectTrigger><SelectValue placeholder="Choose a program" /></SelectTrigger><SelectContent>{context.program_catalog.map((program) => <SelectItem key={program.id} value={program.source_program_id || program.id}>{program.name} · {program.status_label}</SelectItem>)}</SelectContent></Select></div><Button onClick={() => void assignProgram()} disabled={!employerId || !programId || assigning}>{assigning && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{assigning ? "Assigning" : "Assign program"}</Button></div><p className="mt-3 text-xs text-muted-foreground">This assigns the selected program to the employer. Employee access remains gate-controlled.</p></CardContent></Card>
