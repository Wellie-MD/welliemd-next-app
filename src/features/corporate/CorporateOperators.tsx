...
export default function CorporateOperators() {
  ...
  useEffect(() => { void load(); }, []);
  return <div className="space-y-6 p-4 sm:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><Button variant="ghost" size="sm" asChild className="-ml-3 mb-2"><Link to="/dashboard/corporate"><ArrowLeft className="mr-2 h-4 w-4" />Corporate overview</Link></Button><div className="flex items-center gap-2"><h1 className="text-2xl font-bold">Operators</h1><Badge>Corporate pilot</Badge></div><p className="mt-1 text-sm text-muted-foreground">Corporate accounts classified in the control plane.</p></div><Button disabled aria-disabled="true"><Plus className="mr-2 h-4 w-4" />Onboard operator — deferred</Button></div>
