...
export default function CorporateEmployerDashboard() {
  ...
  const { context, metrics, program } = dashboard;
  return <div className="space-y-6 p-4 sm:p-6 lg:p-8">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div>{returnToOperator && <Button variant="ghost" size="sm" onClick={exitEmployer} className="-ml-3 mb-2"><ArrowLeft className="mr-2 h-4 w-4" />Return to operator</Button>}<div className="mb-2 flex items-center gap-2"><Badge>Day 2 live</Badge><Badge variant="outline">Employer Admin context</Badge></div><h1 className="text-2xl font-bold">Corporate Employer Dashboard</h1><p className="mt-1 text-sm text-muted-foreground">Current tenant: {context.name}</p></div><Badge variant="secondary"><ShieldCheck className="mr-1 h-3.5 w-3.5" />{dashboard.privacy}</Badge></div>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.1fr_1fr]"><Card><CardHeader><CardTitle className="text-base">Assigned program</CardTitle></CardHeader><CardContent>{program ? <><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{program.name}</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{program.description}</p></div><Badge>{program.status_label}</Badge></div><div className="mt-5 grid gap-2 sm:grid-cols-3">{program.orientation_modules.map((module) => <div key={module.id} className="rounded-lg border bg-muted/20 p-3"><p className="text-xs font-semibold uppercase text-muted-foreground">{module.kind} · {module.minutes} min</p><p className="mt-1 text-sm font-medium">{module.title}</p></div>)}</div></> : <p className="text-sm text-muted-foreground">No pilot-ready program is assigned.</p>}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">Gate snapshot</CardTitle></CardHeader><CardContent className="space-y-3">{dashboard.gate_snapshot.map((gate) => <div key={gate.number} className="flex items-center justify-between rounded-lg border p-3"><div><p className="font-medium">Gate {gate.number}: {gate.name}</p><p className="text-xs text-muted-foreground">{gate.description}</p></div><span className="text-xl font-bold">{gate.count}</span></div>)}</CardContent></Card></div>
